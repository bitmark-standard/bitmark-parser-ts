...
gap_text:
     BitGapText format CL nl* ( bitElem nl* )*
;
gap_text_instruction_grouped:
    BitGapTextInstructionGrouped format CL nl?
        ( bitElem nl* )*
	NL* ( resource (NL* resource)* )?
;
...
item:   OPITEM (COLON|s_and_w)* CL ( S* lead )*
      | OPC s_and_w? CL ( S* lead )*
;
lead:
        OPLEAD s_and_w (COLON|s_and_w)* CL
      | OPPAGENUMBER s_and_w (COLON|s_and_w)* CL
      | OPMARGINNUMBER s_and_w (COLON|s_and_w)* CL
      | OPC s_and_w? (COLON|s_and_w)* CL
;
