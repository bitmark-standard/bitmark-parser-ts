...
function diffChars(oldStr, newStr, options) {
    return charDiff.diff(oldStr, newStr, options);
}
...
const EXPECTED_JSON_FILEPATH: string = './tests/EXPECTED.JSON';
let expected_content: string = '';
let check_diff: boolean = false;

function load_expected(): void {
    expected_content = fs.readFileSync(EXPECTED_JSON_FILEPATH, { encoding: 'utf8', flag: 'r' });
};

function get_expected(filepath: string): string | null {
    const re: RegExp = new RegExp('<<<<' + filepath + '(\\s+[^<<<<]*\\s+)<<<<', 'm');
...
function __run__(filepath: string, trace: boolean, debug: boolean, bit: string) {
    ...
    let bitmark: parser.BitmarkParser = new parser.BitmarkParser(text, options, bit);
    try {
        ...
        if (json) {
            ...
            if (check_diff) {
                ...
                if (!exp)
                    ...
                    // {ignoreWhitespace: true} ignoreWhitespace is not available in TS
                    let d = diffChars(exp.trim(), json.trim(), {});
                    console.log(d); // new for TS version
...
function __main__(trace: boolean, debug: boolean) {

    let runtest = (filepath: string) => {
        try {
...
let myArgs: Object = process.argv.slice(0, process.argv.length);
