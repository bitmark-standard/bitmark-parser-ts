...
import { BitmarkErrorListener } from './index.ts';

import { Parser } from "../node_modules/antlr4ts/Parser.js";
import { ParserRuleContext } from "../node_modules/antlr4ts/ParserRuleContext.js";
...
class BitmarkListener {
    ...
    // Call on enter
    push_tmpl(ctx: ParserRuleContext, type: string, template: Object = clone(JSON_BIT_TEMPLATES.Regular_bit)) {
    ...
    set_value_based_on_curr_bit_stk(val1: string,
		...
		if (key_obj.length === 2) {
			if (tmpl && !(key_obj[0] in this.stk.top().bit[key_obj[1]]))
				(this.stk.top()).bit[key_obj[1]][key_obj[0]] = clone(tmpl);
    ...
    enterGap(ctx: ParserRuleContext): void {
		...
		if (0 < this.stk.size) {
			...
			let key: string = `{${n}}`;
			this.stk.top().bit['placeholders'][key] = clone(JSON_BIT_TEMPLATES.Gap_bit);
    ...
    enterHeaded_choices(ctx: ParserRuleContext): void {
		let p: Object = clone(JSON_BIT_TEMPLATES.Mct_placeholder);
		let n: number = 0;
		const bit: Object = this.stk.top().bit;
    ...
    enterMcrsep(ctx: ParserRuleContext): void {
		...
		if (bit.type.startsWith('multiple-choice')) {
			let quiz: Object = clone(JSON_BIT_TEMPLATES.Mc_quiz);
			...
			let resp: Object = clone(JSON_BIT_TEMPLATES.Mr_response);
			...
			let stmt: Object = clone(JSON_BIT_TEMPLATES.True_false_stmt);
    ...
    exitChoice_plus(ctx: ParserRuleContext): void {
		...
		if (what === 'bot_action') {
			...
			opt = bit_type !== 'highlight-text' ? clone(JSON_BIT_TEMPLATES.Mct_opt) :
				clone(JSON_BIT_TEMPLATES.Highlight_text_text);
			opt['text'] = val;
			...
			if (1 < spl[0]) {
				...
				if (4 < spl.length) {
					choice_obj = clone(spl[spl.length - 1]);  // clone
    ...
    exitChoice_minus(ctx: ParserRuleContext): void {
		...
		if (what === 'bot_action') {
			...
			if (1 < spl[0]) {
				...
				if (4 < spl.length) {
					choice_obj = clone(spl[spl.length - 1]);
					(bit[spl[1]][sz1 - 1])[spl[2]].push(choice_obj);
					sz2 = (bit[spl[1]][sz1 - 1])[spl[2]].length;
				}
				else {
    ...
    enterMultiple_choice_1(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'multiple-choice-1', clone(JSON_BIT_TEMPLATES.Multi_choice_1));
    };
    ...
    enterMultiple_choice(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'multiple-choice', clone(JSON_BIT_TEMPLATES.Multi_choice));
    };
    ...
    enterMultiple_response_1(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'multiple-response-1', clone(JSON_BIT_TEMPLATES.Multi_response_1));
    };
    ...
    enterMultiple_response(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'multiple-response', clone(JSON_BIT_TEMPLATES.Multi_response));
    };
    ...
    enterMultiple_choice_text(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'multiple-choice-text',
					   clone(JSON_BIT_TEMPLATES.Multi_choice_text));
    };
    ...
    enterHighlight_text(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'highlight-text', clone(JSON_BIT_TEMPLATES.Highlight_text));
    };
    ...
    enterHighlight_inline_choices(ctx: ParserRuleContext): void {
		let code: string = this.but.getcode(ctx);
		let p: Object = clone(JSON_BIT_TEMPLATES.Highlight_placeholder);
		let n: number = Object.keys(this.stk.top().bit['placeholders']).length;
		let key: string = `{${n}}`;
    ...
    enterEssay(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'essay', clone(JSON_BIT_TEMPLATES.Essay_bit));
    ...
    enterInterview(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'interview', clone(JSON_BIT_TEMPLATES.Interview_bit));
    };
    ...
    enterInterview_qanda(ctx: ParserRuleContext): void {
		let qanda: Object = clone(JSON_BIT_TEMPLATES.Interview_question);
    ...
    enterInterview_instruction_grouped(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'interview-instruction-grouped',
					   clone(JSON_BIT_TEMPLATES.Interview_bit));
    };
    ...
    enterMatch_(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'match', clone(JSON_BIT_TEMPLATES.Match_bit));
    }
    ...
    enterMatch_all(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'match-all', clone(JSON_BIT_TEMPLATES.Match_bit));
    }
    ...
    enterMatch_all_reverse(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'match-all-reverse', clone(JSON_BIT_TEMPLATES.Match_bit));
    };
    ...
    enterMatch_reverse(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'match-reverse', clone(JSON_BIT_TEMPLATES.Match_bit));
    }
    ...
    enterMatch_picture(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'match-picture', clone(JSON_BIT_TEMPLATES.Match_bit));
    };
    ...
    enterMatch_audio(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'match-audio', clone(JSON_BIT_TEMPLATES.Match_bit));
    };
    ...
    enterMatch_matrix(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'match-matrix', clone(JSON_BIT_TEMPLATES.MatchMatrix_bit));
    };
    ...
    enterPqpair(ctx: ParserRuleContext): void {
		...
		let p: Object = clone(JSON_BIT_TEMPLATES.Match_pair);
		delete (p['keyImage']);
		delete (p['keyAudio']);
    ...
    enterPair_multival(ctx: ParserRuleContext): void {
		let p: Object = clone(JSON_BIT_TEMPLATES.MatchMatrix_matrixelem);
		delete (p['keyImage']);
		delete (p['keyAudio']);
    ...
    enterPair(ctx: ParserRuleContext): void {
		let p = clone(JSON_BIT_TEMPLATES.Match_pair);
		delete (p.keyImage);
		delete (p.keyAudio);
		this.stk.top().bit['pairs'].push(p);
    };
    ...
    exitImage_chained(ctx: ParserRuleContext): void {
		...
		if (1 < this.curr_bit_stk.size) {
			...
			if (!this.stk.top().bit[parent])
				this.stk.top().bit[parent] = clone(JSON_BIT_TEMPLATES.Imageresource_element);
    ...
    enterMpanswer(ctx: ParserRuleContext): void {
		...
		let cell = clone(JSON_BIT_TEMPLATES.MatchMatrix_cell);
		...
		if (0 < this.stk.size) {
			for (let val of vals)
				if (val.length != 0) {
					...
					(this.stk.top()).bit['body'] =
						replaceFrom((this.stk.top()).bit['body'], val, '', i);
				}
		}
		else
			log['error']("enterValue_title stack is empty");
    ...
    enterMatch_solution_grouped(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'match-solution-grouped', clone(JSON_BIT_TEMPLATES.Match_bit));
    };
    ...
    enterTrue_false_1(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'true-false-1', clone(JSON_BIT_TEMPLATES.True_false_1));
    }
    ...
    enterTrue_false(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'true-false', clone(JSON_BIT_TEMPLATES.True_false));
    }
    ...
    enterSequence(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'sequence', clone(JSON_BIT_TEMPLATES.Sequence));
    };
    ...
    enterMark_(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'mark', clone(JSON_BIT_TEMPLATES.Mark_bit));
    }
    ...
    enterMark_color(ctx: ParserRuleContext): void {
		...
		if (1 < vals.length) {
			this.set_value_based_on_curr_bit_stk(vals[1], 'mark', null, null,
												 clone(JSON_BIT_TEMPLATES.Mark_element));
			// Remove it from the body
			(this.stk.top()).bit['body'] = (this.stk.top()).bit['body'].replace(code, '');
		}
		else
    ...
    enterArticle(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'article', clone(JSON_BIT_TEMPLATES.Article_bit));
    };
    // Enter a parse tree produced by bitmarkParser#statement
    enterStatement(ctx: ParserRuleContext): void {
		// same as article
		this.push_tmpl(ctx, 'statement', clone(JSON_BIT_TEMPLATES.Article_bit));
    };
    enterDetails(ctx: ParserRuleContext): void {
		// same as article
		this.push_tmpl(ctx, 'details-1', clone(JSON_BIT_TEMPLATES.Article_bit));
    };
    enterSample_solution(ctx: ParserRuleContext): void {
		// same as article
		this.push_tmpl(ctx, 'sample-solution', clone(JSON_BIT_TEMPLATES.Article_bit));
    };
    enterWbtcontinue(ctx: ParserRuleContext): void {
		// same as article
		this.push_tmpl(ctx, 'wbt-continue', clone(JSON_BIT_TEMPLATES.Article_bit));
    };
    enterButtoncopytext(ctx: ParserRuleContext): void {
		// same as article
		this.push_tmpl(ctx, 'button-copy-text', clone(JSON_BIT_TEMPLATES.Article_bit));
    };
    enterLearningpathdetails(ctx: ParserRuleContext): void {
		// same as article
		this.push_tmpl(ctx, 'learning-path-details', clone(JSON_BIT_TEMPLATES.Article_bit));
    };
    ...
    enterPage(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'page', clone(JSON_BIT_TEMPLATES.Article_bit));
    };
    //exitPage(ctx):void {};
    ...
    enterMessage(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'message', clone(JSON_BIT_TEMPLATES.Message_bit));
    }
    // NEW 12/2/2020
    //exitMessage(ctx):void {};

    enterChat(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'chat', clone(JSON_BIT_TEMPLATES.Chat_bit));
    };
    ...
    enterConversation(ctx: ParserRuleContext): void {
		this.push_tmpl(ctx, 'conversation', clone(JSON_BIT_TEMPLATES.Chat_bit));
    };
    ...
    exitImage_one(ctx: ParserRuleContext): void {
		...
		let tmpl = key === 'image' ? clone(JSON_BIT_TEMPLATES.Image_detail_element) :
			clone(JSON_BIT_TEMPLATES.ImageLink);
		// From footer
		...
		if (bittype in {
			'chat': 0, 'conversation': 1, 'conversation-left-1': 2, 'conversation-right-1': 3,
		...
		else if (bittype === 'images-logo-grave') {
			const logo_tmpl = clone(JSON_BIT_TEMPLATES.Image_logos_element);
			bit[this.body_key] = bit[this.body_key].replace(code, '');
			const slot = 'logos';
			if (this.stk.top().bit[slot] == undefined)
				bit[slot] = [];  // logos = []
			bit[slot].push(logo_tmpl);
			let index = bit[slot].length-1;
			bit[slot][index]['type'] = what.substr(1).trim();
			bit[slot][index][key][this.resformat === '&image' ? 'src' : 'url'] = url;
    ...
    exitVideo_one(ctx: ParserRuleContext): void {
		...
		if (key in this.stk.top().bit
			...
			bit[slot][key] = key === 'video' ? clone(JSON_BIT_TEMPLATES.Video) : clone(JSON_BIT_TEMPLATES.VideoLink);
    ...
    exitWebsitebit(ctx: ParserRuleContext): void {
		...
		if (what === '&website-link') {
			//
			bit[slot] = clone(JSON_BIT_TEMPLATES.Res_website_link);
			what = what.substr(1);
			bit[slot]['type'] = what;
    ...
    enterLearning_path_book(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-book', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_sign(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-sign', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_video_call(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-video-call', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_learning_goal(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-learning-goal', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_closing(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-closing', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_feedback(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-feedback', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_bot_training(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-bot-training', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_external_link(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-external-link', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_classroom_training(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-classroom-training', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    enterLearning_path_classroom_event(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'learning-path-classroom-event', clone(JSON_BIT_TEMPLATES.LearningPath_bit)); };
    ...
    enterAi_prompt(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'ai-prompt'); this.curr_bit_stk.push('ai-prompt'); };
    exitAi_prompt(ctx: ParserRuleContext): void { this.curr_bit_stk.pop(); };
    enterNote_ai(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'note-ai'); this.curr_bit_stk.push('note-ai'); };
    exitNote_ai(ctx: ParserRuleContext): void { this.curr_bit_stk.pop(); };
    enterSummary_ai(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'summary-ai'); this.curr_bit_stk.push('summary-ai'); };
    exitSummary_ai(ctx: ParserRuleContext): void { this.curr_bit_stk.pop(); };
    enterArticle_ai(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'article-ai'); this.curr_bit_stk.push('article-ai'); };
    ...
    //
    enterCook_preparation(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_preparation'); }
    enterCook_step(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_step'); }
    enterCook_ingredients(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_ingredients'); }
    enterCook_remark(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_remark'); }
    enterCook_variation(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_variation'); }
    enterCook_insert(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_insert'); }
    enterCook_arrangement(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_arrangement'); }
    enterCook_practice_advise(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_practice_advise'); }
    enterCook_plate(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_plate'); }
    enterCook_recommendation(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_recommendation'); }
    enterCook_personal_recommendation(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_personal_recommendation'); }
    enterCook_side_drink(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_side_drink'); }
    enterCook_side_dish(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_side_dish'); }
    enterCook_timer(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'cook_timer'); }
    //
    enterLang_learning_outcomes(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_learning_outcomes'); }
    enterLang_enabling_language_skills(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_enabling_language_skills'); }
    enterLang_life_skills(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_life_skills'); }
    enterLang_english_around_world(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_english_around_world'); }
    enterLang_good_to_know(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_good_to_know'); }
    enterLang_learning_strategy(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_learning_strategy'); }
    enterLang_like_a_local(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_like_a_local'); }
    enterLang_useful_phrases(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_useful_phrases'); }
    enterLang_level_down(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_level_down'); }
    enterLang_level_up(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_level_up'); }
    enterLang_extra_activity(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_extra_activity'); }
    enterLang_video_script(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_video_script'); }
    enterLang_audio_script(ctx: ParserRuleContext): void { this.push_tmpl(ctx, 'lang_audio_script'); }
	...
	enterImage_mood(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'image-mood'); }
	enterImage_figure(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'image-figure'); }
	enterImage_banner(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'image-banner'); }
	enterImage_styled(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'image-styled'); }
	enterImage_screenshot(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'image-screenshot'); }
	enterApp_get_screenshot(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'app-get-screenshot'); }
	enterApp_create_bits_from_image(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'app-create-bits-from-image'); }
	enterLife_skill_sticker(ctx: ParserRuleContext): void  { this.push_tmpl(ctx,'life-skill-sticker'); }
	enterDetails_image(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'details-image'); }
	enterPage_banner(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'page-banner'); }
	enterPage_buy_button(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'page-buy-button'); }

	enterVendor_jsfiddle(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'vendor-jsfiddle'); }
	enterVendor_jsfiddle_embed(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'vendor-jsfiddle-embed'); }
	enterImage_render_svg(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'image-render-svg'); }

	enterApp_code_editor(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'app-code-editor'); }
	enterApp_code_ide(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'app-code-ide'); }
	enterCode_runtime(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'code-runtime'); }
	enterOutput(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'output'); }
	enterConsole_log(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'console-log'); }
	enterVendor_jupyter_cell_code(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'vendor-jupyter-cell-code'); }
	enterVendor_jupyter_cell_raw(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'vendor-jupyter-cell-raw'); }
	enterVendor_jupyter_output(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'vendor-jupyter-output'); }
	enterVendor_jupyter_cell_markdown(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'vendor-jupyter-cell-markdown'); }
	enterVendor_jupyter_ipynb(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'vendor-jupyter-ipynb'); }
	enterApp_code_cell(ctx: ParserRuleContext): void  { this.push_tmpl(ctx, 'app-code-cell'); }
